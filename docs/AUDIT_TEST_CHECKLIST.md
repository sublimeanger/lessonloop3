...
| FIN-INT-03 | P0 | Invoice status transition: `paid` → any other | Refund-driven `paid → sent` allowed when `paid_minor < total_minor`; every other transition out of `paid` blocked (including manual status-only UPDATEs that don't reduce paid_minor) | ✅ Trigger function (post-A4 fix, migration 20260417200000) enforces the invariant `status='paid' implies paid_minor >= total_minor`. A refund that reduces paid_minor below total_minor legitimately reopens the invoice to `'sent'`; recalc cron then flips `sent → overdue` if due_date has passed. |
