...
| J3-F11 | MED | Product UX | Native iOS app shows "visit lessonloop.net" with no escape. Email link likely re-opens app. | `PortalInvoices.tsx:601-606`, `PaymentDrawer.tsx:106-167` | Out of scope — product decision. |
...
| J5-F4 | MED | Multi-child semantics | `WeeklyGoalCard` aggregates days across all children when no filter active. "5 days a week" goal can be met by either child practicing those days. Confusing. | `WeeklyGoalCard.tsx:40-54` | Either require child filter for the card to render, or scope per-child internally. Product decision. |
| J5-F5 | MED | Multi-child semantics | `daysPracticed` set-of-dates conflates children. Alice + Bob both Monday = 1 day. | `WeeklyGoalCard.tsx:50` | Same fix as F4. |
...
1. **J5-F11 — practice_streaks RLS lockdown** — 3-line migration dropping the loose INSERT/UPDATE policies. Closes a vandalism vector (any parent can corrupt any student's streaks in their org). Highest leverage, smallest fix; ship first.
...
- **CC-F3** — Is parent-side LoopAssist supposed to ship? If yes, restore the trigger. If no, remove the dead-mounted component. Affects the answer to whether ParentLoopAssist hooks/edge fn need any other fixes.
- **J3-F11** — Marked "product decision — escalate"; revisit before MED batch cleanup.
- **J5-F4 / J5-F5** — Multi-child weekly goal semantics; either gate the card on child filter or change aggregation. Ask before fixing.
