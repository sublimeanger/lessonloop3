-- FIX 2: Redeem the oldest available credit when a make-up booking is confirmed
-- The confirm_makeup_booking RPC adds the student to the lesson and marks the
-- waitlist entry as booked, but never marks any credit as redeemed.
...
BEGIN
  ...
  VALUES (_org_id, auth.uid(), 'makeup_booked', 'make_up_waitlist', _waitlist_id,
    jsonb_build_object('student_id', _entry.student_id, 'lesson_id', _entry.matched_lesson_id,
      'redeemed_credit_id', _redeemed_credit_id));
