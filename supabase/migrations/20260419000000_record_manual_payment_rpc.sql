-- Fix 1 + Fix 4: billing coherence post-audit (revised)
--
-- Changes in this migration:
--
-- 1. New record_manual_payment RPC — closes direct-INSERT hole.
--    Hard-rejects overpayment (caller is intentional, can correct).
--
-- 2. New invoices.overpayment_minor column — stores excess when
--    net_paid > total_minor. Default 0. Set during recalculate_invoice_paid.
--
-- 3. record_stripe_payment — due-date-aware paid→sent transition at
--    both transition sites (idempotent early-return + main path).
--    DOES NOT hard-reject overpayment: Stripe charges happen before
--    the webhook fires, so a reject here would create reconciliation
--    drift (Stripe charged, LessonLoop has no payment row). Instead
--    the payment is recorded and overpayment_minor reflects the
--    excess on the invoice.
--
-- 4. recalculate_invoice_paid — due-date-aware paid→sent branch.
--    Also now populates overpayment_minor on every recalc.
--
-- Overpayment asymmetry is intentional. Manual: reject at intent.
-- Stripe: accept at fact. Ledger matches reality in both cases.

BEGIN;

-- ============================================================
-- 1. invoices.overpayment_minor column
-- ============================================================
...
-- ============================================================
-- 2. record_manual_payment RPC (new)
-- ============================================================
...
BEGIN
  ...
  INSERT INTO payments (
    ...
    _invoice.org_id,
    p_amount_minor,
    _invoice.currency_code,
    p_method::payment_method,
    'manual'::payment_provider,
    p_reference,
    p_paid_at,
    p_installment_id
  )
...
-- ============================================================
-- 3. record_stripe_payment (due-date-aware, NO overpayment guard)
-- ============================================================
...
BEGIN
  ...
  IF _invoice.status = 'paid' THEN
    -- Paid + new charge = webhook likely duplicate or crossed with refund.
    -- Do not skip silently — still check idempotency below and record if new.
    NULL;
  END IF;
  ...
  IF _existing_payment_id IS NOT NULL THEN
    ...
    IF _invoice.status NOT IN ('void', 'cancelled') THEN
      ...
      ELSIF _invoice.status = 'paid' AND _net_paid < _invoice.total_minor THEN
        -- Refund-reopen: respect due_date
  ...
  -- Insert payment. Note: no overpayment guard — Stripe charge already
...
-- ============================================================
-- 4. recalculate_invoice_paid (due-date-aware + overpayment_minor)
-- ============================================================
