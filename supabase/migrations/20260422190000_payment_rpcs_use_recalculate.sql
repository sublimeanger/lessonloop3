...
--
-- record_stripe_payment and record_payment_and_update_status both
-- computed paid_minor + status + overpayment_minor + installment
-- cascade inline. This duplicated the logic that already lives in
-- recalculate_invoice_paid, which was then improved in Journey 4 to
-- handle the A4 due-date-aware paid→sent/overdue branch and the
-- overpayment_minor column.
--
-- Current state:
--   - record_stripe_payment (20260419000000) already has A4 inline,
--     but duplicated recalc logic — reconsolidate for maintainability.
--   - record_payment_and_update_status (20260418092500) does NOT have
--     A4 and does NOT update overpayment_minor — delegation fixes both.
--
-- Fix: replace the inline status-flip tail in both RPCs with
-- PERFORM recalculate_invoice_paid(_invoice_id). The recalc helper
-- becomes the single source of truth for paid_minor, status
-- transitions, overpayment tracking, and per-installment recalc
-- cascade. Running installment recalc twice (once inline by the
-- cascade, once by the helper) is idempotent — recalculate_installment_status
-- is pure with respect to repeated application.
--
-- Preserves:
--   - Function signatures
--   - Idempotency early-return in record_stripe_payment
--   - Installment cascade in record_payment_and_update_status (still
--     runs before the helper — the helper just finishes the job)
--   - Auth guards, audit_log 'payment_recorded' entry
--   - Existing REVOKE grants (CREATE OR REPLACE preserves ACL)
...
BEGIN
  ...
  -- Cascade across installments: apply as much as possible to each in
  -- order. Partial hits mark the target partially_paid via the helper.
  -- We set payments.installment_id on the FIRST installment touched so
  -- the reverse link exists for dashboards.
  ...
  -- Delegate final paid_minor + status + overpayment_minor + installment
  -- recalc to the single source of truth. Runs installment recalc a
  -- second time on any inline-cascaded rows; idempotent.
...
BEGIN
  ...
  IF _existing_payment_id IS NOT NULL THEN
    -- Still delegate recalc so ledger reflects any refunds that landed
    -- between original charge and this retry. Cheap; recalc is idempotent.
    _recalc_result := recalculate_invoice_paid(_invoice_id);
    RETURN json_build_object(
  ...
  -- Insert payment. No overpayment guard — Stripe charge already
  -- occurred; recording the payment keeps our ledger matching reality.
  -- Overpayment surfaces via invoices.overpayment_minor.
  ...
  -- Lump-sum "pay the rest": iterate pending/overdue/partially_paid.
  -- recalculate_invoice_paid would do this itself below, but leaving
  -- the explicit loop keeps the behaviour identical for callers that
  -- depend on the cascade order under _pay_remaining.
  ...
  -- Single source of truth for paid_minor + status + overpayment_minor
  -- + full installment cascade. Subsumes the previously-inline logic.
  _recalc_result := recalculate_invoice_paid(_invoice_id);
