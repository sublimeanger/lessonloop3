...
--
-- Why:
--   The cron currently has no observability for charge attempts and no pause
--   mechanism — it retries indefinitely under the same idempotency key, spamming
--   parents and giving operators no operator surface. This migration adds:
--     (a) auto_pay_attempts: a row per cron-attempted charge regardless of outcome
--     (b) pause columns on guardian_payment_preferences so the cron can stop
--         charging a guardian after consecutive failures.

-- Per-attempt log. One row per cron-attempted charge regardless of outcome.
...
-- Service role writes only — no INSERT/UPDATE/DELETE policy for authenticated.
...
-- Extend message_log.source CHECK to allow 'auto_pay_cron' (used by the
-- new send-auto-pay-failure-notification and send-auto-pay-alert fns
-- introduced in C2 and C4).
