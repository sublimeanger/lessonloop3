...
BEGIN
  ...
  -- Lock and sum credit values atomically (FOR UPDATE prevents double-spend)
  IF array_length(_credit_ids, 1) > 0 THEN
    ...
    -- Validate all requested credits were found and unredeemed
  ...
  IF array_length(_credit_ids, 1) > 0 THEN
    ...
    SET redeemed_at = NOW(),
        notes = 'Applied to invoice_id:' || _invoice.id::text
    WHERE id = ANY(_credit_ids)
      AND org_id = _org_id
      AND redeemed_at IS NULL;
  END IF;
