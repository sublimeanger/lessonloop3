...
--
-- The retry RPC re-attempts ONLY students who errored in the parent
-- run, using the parent run's stored period (NOT template.next_run_date,
-- which the parent run already advanced).
--
-- Per Phase 2 design doc + Phase 4B walk findings F4 + RPC scoping notes.
-- Idempotent via CREATE OR REPLACE and ADD COLUMN IF NOT EXISTS.

-- ─── Schema: parent_run_id linkage ──────────────────────────────────
...
BEGIN
  ...
  -- Auth gate (mirrors generate_invoices_from_template).
  ...
  -- Refuse cancelled runs (semantically unclear — operator should
  -- create a new manual run instead).
  ...
  -- Defensive: refuse if parent is still running.
  IF _parent.outcome = 'running' THEN
    RAISE EXCEPTION 'Cannot retry run % while it is still running.', _run_id
      USING ERRCODE = '22023';
  END IF;

  -- Load template (no FOR UPDATE — retry is read-only on template state
  -- aside from last_run_id pointer at the end).
  ...
  -- ─── Collect failed student_ids from parent run ─────────────────
  -- Distinct + non-NULL (template-level errors like no_next_term have
  -- NULL student_id and shouldn't be retried per-student).
  SELECT ARRAY(
  ...
  -- ─── Create new run row linked to parent ────────────────────────
  INSERT INTO recurring_template_runs (
    ...
    _parent.period_start, _parent.period_end,  -- reuse parent's period
    'retry', auth.uid(),
    'running',
  ...
  -- ─── Per-failed-recipient loop ──────────────────────────────────
  FOREACH _student_id IN ARRAY _failed_student_ids LOOP
    BEGIN  -- savepoint per recipient

      -- ── 1. Resolve payer (same chain as generator) ────────────
      ...
      -- ── 2a. Upfront/hybrid pre-check: skip if already invoiced ─
      -- For upfront/hybrid, check if a non-void invoice exists for
      -- this student linked to ANY run for this template covering
      -- the same period. Prevents double-billing across retries.
      IF _template.billing_mode IN ('upfront', 'hybrid') THEN
        IF EXISTS (
          ...
          WHERE i.generated_from_template_id = _template.id
            ...
            -- Match by run period overlap (the parent's period).
            AND EXISTS (
      ...
      -- ── 2b. Resolve billable items ───────────────────────────
      _billable_items := '[]'::jsonb;
      _recipient_had_items := false;
      _no_rate_count := 0;

      -- Delivered or hybrid: lesson-backed.
      -- Delivered-mode dedup is handled by the partial unique index
      -- on invoice_items.linked_lesson_id WHERE NOT NULL.
      ...
      -- Upfront or hybrid: fixed items (already cleared the "already
      -- invoiced" gate above).
      IF _template.billing_mode IN ('upfront', 'hybrid') THEN
        FOR _item_row IN
  ...
  -- Update template's last_run pointer ONLY if this retry produced
  -- something. Otherwise the failed retry would overwrite the parent
  -- run's last_run reference.
