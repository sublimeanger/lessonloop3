-- J9 Phase 2 C1 — CIWI auth guard service-role carve-out.
-- Prerequisite for generator RPC (C4) which calls CIWI from inside
-- SECURITY DEFINER context with service-role session (auth.uid() IS NULL).
--
-- Current guard: IF NOT is_org_finance_team(auth.uid(), _org_id) THEN RAISE
-- Amended guard: IF auth.uid() IS NOT NULL
--                   AND NOT is_org_finance_team(auth.uid(), _org_id)
--                THEN RAISE
--
-- Semantics: when auth.uid() IS NULL (service role caller reaching
-- CIWI via SECURITY DEFINER wrapper), skip the row-level check.
-- Service-role callers reach CIWI ONLY through trusted in-DB wrappers
-- which must implement their own auth gate. See design doc Appendix A.
--
-- Every other aspect of CIWI preserved byte-for-byte from the prior
-- definition in 20260316260000_fix_voided_credits_audit.sql.
