-- ============================================================
-- CRD-H1 HIGH: Use actual academy term dates for credit cap
-- CRD-H2 HIGH: Fall back to lesson participant rate when no
--              invoice item or rate card exists for credit value
-- ============================================================
--
-- CRD-H1: The max-credits-per-term check used date_trunc('quarter', CURRENT_DATE)
-- which is calendar quarters (Jan-Mar, Apr-Jun...), NOT the academy's actual terms.
-- An academy with Sep-Dec terms had the cap reset mid-term at Jan 1.
-- Fix: Look up the actual current term from the terms table.
--
-- CRD-H2: The credit value fallback chain was: invoice_items → student rate card
-- → org default rate card. But lesson_participants.rate_minor (snapshot at lesson
-- creation) was not consulted. This adds it as a third fallback.
