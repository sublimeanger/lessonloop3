...
--
-- Supersedes: supabase/migrations/20260516100000_canary_walk_batch_1z_combined_fixes.sql
--   The previous file's CW-F4 section combined `INSERT OR UPDATE OR
--   DELETE` with `REFERENCING NEW TABLE / OLD TABLE` on a single
--   trigger. Postgres rejects this with error 0A000 ("transition
--   tables cannot be specified for triggers with more than one
--   event"). The migration was rolled back twice in production
--   attempts. This corrected version splits each surface into three
--   event-specific triggers (INSERT/UPDATE/DELETE), each declaring
--   only the transition tables relevant to its event. The pattern
--   was verified end-to-end against PostgreSQL 16.13 before this
--   migration shipped (3-row INSERT bumps pdf_rev by exactly 1;
--   2-row UPDATE +1; 1-row DELETE +1; 0-row operations bump 0;
--   idempotent on re-apply).
--
-- Sections 1, 2, 4, 5 are byte-identical to the broken file —
-- already idempotent (CREATE OR REPLACE FUNCTION + count guards on
-- backfill blocks + IF NOT EXISTS on the CHECK constraint), safe to
-- re-run regardless of partial-state from the previous failed apply.
--
-- All five sections wrapped in BEGIN/COMMIT — failure in any section
...
-- ============================================================
-- 3. CW-F4 — Convert pdf_rev item triggers to STATEMENT-level (CORRECTED)
-- ============================================================
-- Why
-- ---
...
--
-- CORRECTION FROM PR #378 / 20260516100000_canary_walk_batch_1z_combined_fixes.sql:
-- Postgres does NOT permit a single trigger to combine
-- `INSERT OR UPDATE OR DELETE` with `REFERENCING NEW TABLE / OLD TABLE`
-- (error 0A000: "transition tables cannot be specified for triggers
-- with more than one event"). The original Batch 1Z migration used
-- one combined trigger per surface and was rejected by Postgres.
--
-- Correct shape: three separate triggers per surface (one for INSERT,
-- one for UPDATE, one for DELETE), each declaring only the transition
-- tables relevant to its event. INSERT triggers see _new_rows only;
-- DELETE triggers see _old_rows only; UPDATE triggers see both.
-- Three trigger functions per surface (one per event type), each
-- referencing only the transition table its trigger declares.
--
-- Verified empirically against PostgreSQL 16.13 with a behaviour
-- battery: 3-row INSERT → +1; 2-row UPDATE → +1; 1-row DELETE → +1;
-- 0-row UPDATE → +0; 0-row DELETE → +0; per-surface tests on
-- invoice_installments and payments also +1 per insert. Idempotent
-- re-apply tested.
--
-- Idempotent: DROP TRIGGER IF EXISTS + CREATE TRIGGER + CREATE OR REPLACE.
-- The original per-row functions (bump_invoice_pdf_rev_from_items etc.)
-- remain in pg_proc as no-longer-referenced; harmless residue, can be
-- DROPped in a future cleanup batch.

-- ----------------------------------------------------------------
-- 3a. invoice_items (3 functions, 3 triggers)
-- ----------------------------------------------------------------
...
-- ----------------------------------------------------------------
-- 3b. invoice_installments (3 functions, 3 triggers)
-- ----------------------------------------------------------------
...
-- ----------------------------------------------------------------
-- 3c. payments (3 functions, 3 triggers)
-- ----------------------------------------------------------------
...
-- ----------------------------------------------------------------
-- 3d. PUBLIC permissions for the 9 new functions
-- ----------------------------------------------------------------
