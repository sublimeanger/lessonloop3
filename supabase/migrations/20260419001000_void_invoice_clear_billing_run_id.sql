-- Fix 2 (B28): void_invoice clears billing_run_id and handles
-- partially_paid installments defensively.
--
-- Problem: current void_invoice does not NULL out invoices.billing_run_id
-- when voiding. Means a voided invoice remains attached to its historical
-- billing run, can skew run totals / dedup logic. Zero live manifestation
-- today but latent and will fire the first time anyone voids a
-- previously-billed invoice.
--
-- Also defensively includes 'partially_paid' in the installment void set.
-- This path is unreachable given A5 void guard (paid_minor > 0 blocks
-- void before this code runs), but closing the theoretical gap costs
-- nothing and makes the function's intent explicit.
--
-- Left alone: audit_log entity_type 'invoice' (singular). Tonight's
-- Section 9 audit flagged a plural/singular inconsistency across RPCs
-- and triggers (C50). That consolidation is a dedicated migration.
