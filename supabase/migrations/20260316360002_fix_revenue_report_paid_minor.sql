-- FIN-02: Revenue RPC sums total_minor for status='paid' instead of paid_minor
-- Fix: use paid_minor which reflects actual money received (handles partial payments)
...
AS $$
  ...
  -- Current period: sum paid_minor (actual revenue received)
  SELECT
    'current'::text AS period,
    to_char(issue_date, 'YYYY-MM') AS month,
    COALESCE(SUM(paid_minor), 0)::bigint AS paid_amount_minor,
  ...
  -- Previous period: sum paid_minor (actual revenue received)
  SELECT
