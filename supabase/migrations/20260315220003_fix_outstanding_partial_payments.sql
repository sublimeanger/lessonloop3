-- Fix outstanding calculation to subtract paid_minor for partial payments.
-- Previously summed total_minor for outstanding invoices, ignoring any
...
BEGIN
  ...
  RETURN (
    SELECT json_build_object(
      ...
      'overdue', COALESCE(SUM(CASE WHEN status = 'overdue' OR (status = 'sent' AND due_date < CURRENT_DATE) THEN total_minor - COALESCE(paid_minor, 0) ELSE 0 END), 0),
