...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Pass token explicitly — getUser() no-args calls /auth/v1/user which
    // on this project rejects legacy HS256 JWTs. Same pattern as
...
serve(wrapEdgeFn("send-cancellation-notification", handler));
