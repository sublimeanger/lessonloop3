...
const SAFE_MESSAGES: SafeErrorMap = {
  exact: {
    ...
    "Online payments are disabled for this organisation. Please pay by bank transfer.": 400,
  },
  prefix: {
...
serve(wrapEdgeFn("stripe-create-checkout", async (req) => {
  ...
  try {
    ...
    const { stripe } = await getStripeClient(invoice.org_id, supabase);
    ...
    return classifyAndRespond(error, SAFE_MESSAGES, corsHeaders, "stripe-create-checkout");
  }
}));
