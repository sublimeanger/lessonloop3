...
test.describe('Students — Owner', () => {
  ...
  test('page loads with student list', async ({ page }) => {
    ...
    await expect(page.getByText(/emma|james|sophie/i).first()).toBeVisible({ timeout: 15_000 });
  });
  ...
  test('status filter pills (All/Active/Inactive)', async ({ page }) => {
    ...
    await expect(pills).toBeVisible({ timeout: 5_000 });
  ...
  test('export button visible', async ({ page }) => {
    await goTo(page, '/students');
    // Export button text may be hidden on small viewports (hidden sm:inline)
