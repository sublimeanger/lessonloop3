...
**Phase 1 — Foundation (closed).** Five commits. No consumer wiring.
...
**Filed for later:** **J11-F2 — bucket retention.** `{org_id}/{invoice_id}_<old_rev>.pdf` objects accumulate when `pdf_rev` increments. A cron sweep mirrored on `cleanup-webhook-retention` would purge superseded objects.
