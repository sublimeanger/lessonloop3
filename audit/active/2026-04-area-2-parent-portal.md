...
| J3 | PortalInvoices | 611 | walked | 31 (6 withdrawn) | 6 |
...
**HIGH count: 20 raw findings → 17 distinct fix briefs.** Two duplicates merged (J1-F17 ≡ J3-F7 money-math; J6-F4 + J6-F5 are facets of one RLS migration), plus J5-F11 + J6-F4/F5 + J8-F9 are three separate RLS-lockdown migrations. J3-F19b and J3-F19c filed and closed together with J3-F19 in Batch 2F as a single consolidation refactor (`_shared/invoice-amount-due.ts`); they share one fix shape but are tracked as three separate findings. J3-F19d (staff-side React mirror) filed and closed in Batch 2F-followup as a two-line diff bringing the CC-8 cluster to full closure across every surface in the app.
...
| ~~J1-F4~~ | ~~**HIGH**~~ | ~~Silent data integrity~~ | ~~Parent cancel-booked-makeup does raw UPDATE on `make_up_waitlist` (status `booked → waiting`). Doesn't delete the underlying `lesson_participants` row, so the `on_makeup_participant_removed` trigger (which restores credit + writes audit) never fires. Result: parent loses make-up credit + ghost lesson stays on teacher's calendar.~~ | ~~`PortalHome.tsx:220-232`~~ | ~~Create `cancel_booked_makeup(_waitlist_id)` SECURITY DEFINER RPC that DELETEs the `lesson_participants` row inside a transaction. The existing trigger does the rest.~~ [shipped 2026-04-29, this PR — new SECURITY DEFINER RPC `cancel_booked_makeup` in migration `20260513100000` deletes the matched `lesson_participants` row inside a transaction; existing `trg_makeup_participant_removed` restores credit and resets waitlist; new RPC also writes a parent-intent `'makeup_booking_cancelled'` audit entry. `PortalHome.executeCancelMakeup` now calls the RPC] |
...
| ~~J1-F15~~ | ~~**HIGH**~~ | ~~Hardcoded currency~~ | ~~`formatCurrencyMinor(entry.offered_rate_minor, 'GBP')` — hardcoded GBP. Wrong for any non-UK org.~~ | ~~`PortalHome.tsx:784`~~ | ~~Use `currencyCode`. Cross-cutting sweep also needed.~~ [shipped 2026-04-29, Lovable commit 37163c52 — formatCurrencyMinor at PortalHome.tsx:784 now passes currencyCode variable instead of 'GBP' literal] |
...
| ~~J1-F29~~ | ~~**HIGH**~~ | ~~Audit gap~~ | ~~`respond_to_makeup_offer` RPC writes no `audit_log` entry. Combined with J1-F1, parent accept/decline of make-up offers has zero audit coverage.~~ | ~~migration `20260315210005`~~ | ~~Add `INSERT INTO audit_log` inside the SECURITY DEFINER RPC. Same fix as J1-F1.~~ [shipped 2026-04-29, this PR — `respond_to_makeup_offer` replaced in migration `20260513100000`; both accept and decline branches now write `audit_log` from inside the SECURITY DEFINER function] |
...
| ~~J1-F31~~ | ~~**HIGH**~~ | ~~Multi-org silent failure~~ | ~~`respond_to_makeup_offer` and `cancel_booked_makeup` resolve guardian via `WHERE user_id = auth.uid() AND deleted_at IS NULL LIMIT 1` with no org filter. For parents with rows in multiple orgs, the lookup may bind to the wrong guardian and the subsequent waitlist-ownership check fails with "Waitlist entry not found or does not belong to you" — silent failure, no UX explains why.~~ | ~~`20260315210005_fix_parent_respond_atomic.sql` and `20260513100000_makeup_flow_integrity_j1_f4_f29.sql`~~ | ~~Read waitlist row first to obtain `org_id`, then filter guardian lookup by both `user_id` and `org_id`. Add defensive `IS DISTINCT FROM` check after lookup.~~ [shipped 2026-04-29, this PR — both RPCs replaced in migration `20260514100000`; guardian lookup now filters `g.org_id = _entry.org_id`; defensive `_entry.guardian_id IS DISTINCT FROM _guardian_id` raise after; new error message `Guardian record not found for this organisation` surfaces multi-org case explicitly] |
...
| ~~J2-F24~~ | ~~**HIGH**~~ | ~~Schema drift~~ | ~~`parent_reschedule_policy_override` on `locations` is in the schema with CHECK constraint, but **nobody reads it**. Lessons at locked-down locations are still rescheduleable from the portal.~~ | ~~`PortalSchedule.tsx:127-129`; `locations.parent_reschedule_policy_override`~~ | ~~Resolve effective policy as `location.override ?? org.parent_reschedule_policy`. Apply per-lesson rather than once for the whole page.~~ [shipped 2026-04-29, this PR — `useParentLessons` query extended to fetch `location.parent_reschedule_policy_override`; `ParticipantLesson` + `ParentLesson` types extended; `PortalSchedule.tsx` page-level `reschedulePolicy/canReschedule/showSlotPicker` vars removed; `resolveReschedulePolicy(lesson)` resolver added; `LessonCard` destructures the per-lesson policy at the top of its render. Frontend-only fix; no migration, no edge fn change.] |
...
| ~~J3-F2~~ | ~~LOW~~ | ~~Currency~~ | ~~Hardcoded GBP fallback.~~ | ~~`PortalInvoices.tsx:215`~~ | ~~Use derived constant.~~ [shipped 2026-04-29, Lovable commit 37163c52 — fallback dropped at PortalInvoices.tsx:215; same pattern applied to lines 226, 239, 304, 380, 393, 420, 432, 462 in same commit (closes J3-F10)] |
...
| ~~J3-F7~~ | ~~**HIGH**~~ | ~~Money math~~ | ~~`useParentInvoices` filters on `payer_guardian_id` only. Mixed-payer households: invoices billed to student directly never appear in the list. **Same fix as J1-F17.**~~ | ~~`useParentPortal.ts:382`~~ | ~~`.or('payer_guardian_id.eq.X,payer_student_id.in.(child_ids)')`.~~ [shipped 2026-04-29, this PR — student_guardians lookup added before invoices query; OR-filter applied when studentIds non-empty, fall-back to guardian-only `.eq()` when empty] |
...
| ~~J3-F10~~ | ~~LOW~~ | ~~DRY~~ | ~~`currentOrg?.currency_code || 'GBP'` repeated 8 times.~~ | ~~`PortalInvoices.tsx` various~~ | ~~Derived constant at top.~~ [shipped 2026-04-29, Lovable commit 37163c52 — all 8 repetitions in PortalInvoices.tsx replaced with canonical `?? ''` pattern; also fixed adjacent callsite at line 462 surfaced when PaymentDrawer prop was made required] |
...
| J3-F15 | LOW | Branding | Stripe Elements colorPrimary hardcoded `#2563eb`; `org.brand_color` exists. | `PaymentDrawer.tsx:218` | Pull from `currentOrg.brand_color`. [partially shipped 2026-04-29, Lovable commit 37163c52 — currency hardcoding closed (PaymentDrawer's `currencyCode = 'GBP'` default param removed; prop made required); brand_color part at PaymentDrawer.tsx:218 still pending, separate fix] |
...
| ~~J3-F19d~~ | ~~**HIGH**~~ | ~~Refund netting (staff side)~~ | ~~Two staff-side React surfaces (`RecordPaymentModal.tsx:75-76` and `InvoiceDetail.tsx:188-189`) compute "outstanding"/"amount due" from raw `invoice.payments?.reduce(...)` instead of canonical `invoice.paid_minor`. Surfaced by Batch 2F's grep-D sweep (Rule 3.6(d) halt). Same bug class as J3-F19/J3-F19b/J3-F19c.~~ | ~~`src/components/invoices/RecordPaymentModal.tsx:75-76`, `src/pages/InvoiceDetail.tsx:188-189`~~ | ~~Replace inline `payments?.reduce(...)` with `invoice.paid_minor ?? 0`. Same fix pattern as J3-F19b in PDF renderers.~~ [shipped 2026-05-02, this PR — Batch 2F-followup] |
...
| ~~CC-F7~~ | ~~LOW~~ | ~~Money-math (CC-2 family)~~ | ~~`parent-loopassist-chat` invoices query uses `payer_guardian_id` only — same blindspot as J3-F7 / J1-F17. Parent gets a misleading invoice list from the AI ("you have 3 outstanding invoices") missing student-payer cases.~~ | ~~`parent-loopassist-chat/index.ts:169-176`~~ | ~~CC-2 fix needs to apply here too. Either parameterise the query helper or duplicate the OR-branch logic.~~ [shipped 2026-04-29, this PR — invoices `.in("payer_guardian_id", guardianIds)` replaced with `.or(...)` using both `guardianIds` and `studentIds` already in scope at the call site] |
| ~~CC-F8~~ | ~~LOW~~ | ~~Currency (CC-3 family)~~ | ~~Hardcoded GBP fallback `cc = orgData?.currency_code \|\| "GBP"` and currency symbol mapping in chat.~~ | ~~`parent-loopassist-chat/index.ts:247-248`~~ | ~~CC-3 sweep.~~ [shipped 2026-04-29, this PR — validate-and-fail-loudly + Record-based symbol map] |
...
| ~~CC-F15~~ | ~~**HIGH**~~ | ~~RLS mutual recursion~~ | ~~`term_continuation_runs` and `term_continuation_responses` parent SELECT policies subquery each other's table. PR #367 (`20260511100000_lockdown_parent_rls_j6_j8.sql`) introduced the second half by adding `Parents can view runs they have responses in` while leaving `Parents see own responses` (from `20260315200500`) intact. Production-live HTTP 500 (Postgres 42P17) on every request that touched either table. Parent portal blank-renders for all users; staff dashboards too.~~ | ~~`20260315200500_fix_parent_rls_continuation_org_scope.sql` + `20260511100000_lockdown_parent_rls_j6_j8.sql`~~ | ~~Two SECURITY DEFINER recursion-breaker helpers (`user_has_continuation_response_in_run`, `continuation_run_org_id`); rewrite both parent SELECT policies to call them. SECURITY DEFINER bypasses RLS in the function body so the cross-table read does not re-trigger the other policy.~~ [shipped 2026-04-29, this PR — migration `20260514100000`] |
| ~~portal-defense-1~~ | ~~MED~~ | ~~Defensive rendering~~ | ~~A 500 from any single hook in the parent portal blanks the entire page tree because no `SectionErrorBoundary` wraps section-level consumers. Surfaced by CC-F15.~~ | ~~`src/pages/portal/PortalHome.tsx`, `src/pages/portal/PortalContinuation.tsx`~~ | ~~Wrap `useParentContinuationPending` consumer in `SectionErrorBoundary`. Future batches should extend this to other portal hooks where appropriate.~~ [shipped 2026-04-29, this PR — both portal pages now wrap continuation rendering in `<SectionErrorBoundary name="continuation">`] |
...
- ~~`respond_to_makeup_offer` (RPC) → audit accept/decline~~ [shipped 2026-04-29, this PR — migration `20260513100000`]
- ~~`cancel_booked_makeup` (new RPC, see J1-F4 fix) → audit cancel~~ [shipped 2026-04-29, this PR — migration `20260513100000`]
...
**Also:** add audit triggers for `make_up_waitlist` and `term_continuation_responses` themselves (Track 0.1 P1 missed make_up_waitlist; continuation never had audit coverage). The audit trigger on `guardian_payment_preferences` already exists from `20260426174938` — confirm it's recording the events listed above. [partial: `make_up_waitlist` + `term_continuation_responses` triggers shipped 2026-04-29, this PR — migration `20260513100100` (`audit_make_up_waitlist`, `audit_term_continuation_responses`); Stripe edge fn audit gaps + continuation-respond audit remain]
...
[shipped 2026-04-29, this PR — three fix surfaces; `outstandingMap` not modified per data-flow analysis (per-installment for one specific invoice; corrects automatically once `useParentInvoices` returns the right set)]

### CC-3 — Hardcoded currency
**Findings:** J1-F15 (HIGH), J3-F2 / F10 / F15 (LOW)
**Pattern:** Some places hardcoded `'GBP'` instead of using `currentOrg.currency_code`. claude.md rule explicit.
**Fix shape:** ~~Single grep/sed pass.~~ Closed across two PRs:
- **Portal portion (HIGH J1-F15, LOW J3-F2/F10/F15-currency)** — shipped 2026-04-29 (Lovable commit 37163c52) across 4 portal files / 13 sites.
- **Broader sweep (LOW + cross-cutting)** — shipped 2026-04-29, this PR. Pre-flight baseline 123 occurrences across 75 files; post-sweep 45 occurrences across 17 files (12 Bucket A keeps + 5 seed-demo/onboarding-setup keeps + OrganisationTab inline-justified picker). Closed CC-F8 mechanically. 39 display sites (B-1) moved to `currentOrg?.currency_code ?? ''`; 20 edge functions (B-2) moved to validate-and-fail-loudly. [shipped 2026-04-29, this PR]
...
### ~~CC-8 — Refund netting drift~~ [fully closed 2026-05-02 across all five surfaces — Batch 2F + Batch 2F-followup]
~~**Findings:** J3-F19, J3-F19b (NEW), J3-F19c (NEW), J3-F19d (NEW)~~
~~**Pattern:** Code reimplemented "total paid" math without consulting `invoice.paid_minor` (which is canonical, refund-netted via `recalculate_invoice_paid`).~~
~~**Fix shape:** shipped 2026-05-02 in Batch 2F (four edge-fn / PDF surfaces via consolidation into `supabase/functions/_shared/invoice-amount-due.ts`) and Batch 2F-followup (two staff-side React surfaces inline `invoice.paid_minor ?? 0`). The audit's anticipation that "other edge functions / RPCs that compute amount due could surface more" was correct; the staff-side React surfaces also surfaced under Batch 2F's grep-D Rule 3.6(d) halt and were re-classified as J3-F19d (re-anchored from the original Area-1 filing because they are CC-8 same-bug-class — the J3 cluster is the right home).~~
...
0. ~~**CC-F15 — term_continuation RLS recursion fix**~~ [shipped 2026-04-29, this PR — production-live blocker fixed]
...
15. ~~**J1-F31 — multi-org guardian scope in make-up RPCs**~~ [shipped 2026-04-29, this PR]
