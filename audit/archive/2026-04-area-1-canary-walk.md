...
| ~~CW-F11~~ | ~~LOW~~ → NO-OP | `void_invoice` | ~~References nonexistent `'partially_paid'` installment status~~ — **revised 2026-05-02:** `partially_paid` is a real status that exists in production data (e.g. inst #3 of LL-2026-00008). The branch is correctly defensive; no fix needed. Revisit alongside CW-F12. |
| CW-F12 | **HIGH** | `invoice_installments.status` schema | Column is plain `text` with no enum / CHECK / domain. Any caller can write any string. Root enabling condition for the pre-CW-F9 `status='paid'`-without-attached-payment shape on LL-2026-00008. Filed for Track 0.X. |
| ~~CW-F13~~ | LOW | CW-F9 backfill | ~~Backfill skipped legacy `status='paid'`-without-payment installments~~ [shipped 2026-05-02, one-row remediation `20260501223523_…_cw_f9_ll_2026_00008_remediation.sql`] |

## Severity rollup (final)

- HIGH: 3 / 4 closed (CW-F2 NOT VALID + awaiting VALIDATE; CW-F3, CW-F9 closed in Batch 1Z; CW-F12 NEW, deferred to Track 0.X)
- MED: 2 / 2 closed (CW-F1, CW-F4 in Batch 1Z)
- LOW: 2 / 5 closed (CW-F6 in Batch 1Z; CW-F13 in remediation 2026-05-02; CW-F5, CW-F7, CW-F10 deferred to Track 0.X). CW-F11 re-characterised to NO-OP (defensive branch handles a real status).
...
- **2026-05-02 — Session 6 (Batch 1Z follow-on: CW-F9 one-row remediation + Step 4 sanity):** Post-Batch-1Z verification surfaced one residual drift row: invoice LL-2026-00008 (`d7041eeb-…`) had `paid_minor=5000` but `sum(paid installments)=3333`. Forensics: a pre-CW-F9 legacy code path had marked inst #1 `status='paid'` without attaching a payment row; CW-F9's backfill `WHERE status IN ('pending','overdue','partially_paid')` skipped inst #1 and allocated the 5000 cash to inst #2 (3333) + inst #3 (1667); downstream `recalculate_installment_status` then demoted inst #1 to `pending` (zero attached payments). One-row remediation `20260501223523_cw_f9_ll_2026_00008_remediation.sql` reattached the two payment rows to inst #1 + inst #2 and re-ran `recalculate_installment_status` on all three; final state matches expected (#1 paid 3333/3333, #2 partially_paid 1667/3333, #3 pending 0/3334). Corrected ledger invariant (now accounting for `partially_paid` cash) returns 0 rows. Step 4 trigger sanity `20260501223524_cw_f4_trigger_sanity_test.sql` ran against the same invoice: UPDATE on each of `invoice_items`, `invoice_installments`, `payments` bumped `pdf_rev` by exactly 1; `pdf_rev` restored to its initial value (12) at the end — self-cleaning. Three findings filed: **CW-F11** re-characterised LOW → NO-OP (`partially_paid` is a real status, defensive branch is correct); **CW-F12** NEW HIGH (`invoice_installments.status` is unconstrained `text` — root enabling condition for the original LL-2026-00008 corruption; deferred to Track 0.X); **CW-F13** NEW LOW (CW-F9 backfill design assumption — `status='paid'` rows have attached payments — failed for one legacy row; one-off remediation shipped, no code fix needed because `record_manual_payment` going forward no longer produces this shape). **Batch 1Z fully closed.**
