      ...
      <DialogContent className="h-screen w-screen max-w-none overflow-y-auto rounded-none border-0 p-4 sm:h-auto sm:max-w-xl sm:rounded-lg sm:border sm:p-6">
        ...
        {step === 'complete' && confirmResult && (
          <div className="space-y-4">
            ...
            {(confirmResult as any).waitlist_matches?.length > 0 && (
              <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-center">
                <p className="text-sm font-medium text-emerald-800">
                  {(confirmResult as any).waitlist_matches.length} {(confirmResult as any).waitlist_matches.length === 1 ? 'family' : 'families'} on the waiting list may match this freed slot
                </p>
                <Button
                  variant="link"
                  size="sm"
